...
export const lvcStatusSchema = z.enum(['verified', 'failed', 'needs_attention']);
...
export const enqueueForLegalReviewSchema = z.object({
  ...
  queue_status: z.enum(['pending_review', 'in_review', 'needs_attention']).optional(),
  lvc_status: lvcStatusSchema.optional(),
  missing_evidence: z.array(z.string()).optional(),
  lvc_confidence_score: z.number().int().min(0).max(100).optional(),
});
