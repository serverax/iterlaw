...
| 11 | Local LLM gateway + model routing + bounded synthesis | **PLANNED / READY FOR LOCAL IMPLEMENTATION** — default-disabled gateway interface + bounded-synthesis guard already in repo (`apps/legal-orchestrator/src/legal/llm/`); local model router + citation-bound prompt + LLM output guard pending; live gateway DISABLED / MOCK-SAFE; real deployment **BLOCKED** until Sprint 10 real staging DB verification passes; production **BLOCKED**. | See [`SPRINT_11_LOCAL_LLM_GATEWAY_PLAN.md`](SPRINT_11_LOCAL_LLM_GATEWAY_PLAN.md) + [`SPRINT_11_IMPLEMENTATION_CHECKLIST.md`](SPRINT_11_IMPLEMENTATION_CHECKLIST.md) + `docs/iterlaw/SPRINT_11_LOCAL_LLM_GATEWAY_PLAN.md` (long-form) + `docs/benchmarks/SPRINT_11_LOCAL_LLM_BENCHMARK_EXECUTION_CHECKLIST.md` |
