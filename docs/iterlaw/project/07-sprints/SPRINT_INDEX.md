...
| 11 | Local LLM gateway + model routing | **PLANNED — BLOCKED BY SPRINT 10 STAGING DB VERIFICATION** — implementation NOT STARTED; production BLOCKED. Default-disabled gateway interface + bounded-synthesis guard already in repo (`apps/legal-orchestrator/src/legal/llm/`); planning documents only this sprint. | See [`SPRINT_11_LOCAL_LLM_GATEWAY_PLAN.md`](SPRINT_11_LOCAL_LLM_GATEWAY_PLAN.md) + [`SPRINT_11_IMPLEMENTATION_CHECKLIST.md`](SPRINT_11_IMPLEMENTATION_CHECKLIST.md) + `docs/benchmarks/SPRINT_11_LOCAL_LLM_BENCHMARK_EXECUTION_CHECKLIST.md` |
